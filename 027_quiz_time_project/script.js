...
const currentQuestionNumberSpan = document.querySelector('.current-question-number')
...
const correctPointerWhole = document.querySelector('.correct-pointer-setup-container')
const wrongPointerWhole = document.querySelector('.wrong-pointer-setup-container')
const correctPercentageText = document.querySelector('.correct-percentage-text')
const wrongPercentageText = document.querySelector('.wrong-percentage-text')
const lineGraphCorrectBar = document.querySelector('.linear-graph-fg-green-div')
const resultMotivationalQuote = document.querySelector('.result-motivational-quote')
...
let defaultQuizzes = [
    {
        ...
        quizQuestions: [
            ...
            },
            {
                ...
                answers: ['console.log("Hello World")', 'print("Hello World")', 'echo("Hello World")', 'log("Hello World")'],
            ...
            },
            {
                ...
                answers: ['Changes a variable\'s type', 'Returns the data type of a variable', 'Declares a type', 'Compares two values'],
                correctAnswerIndex: 1,
                selectedIndex: -1,
                seconds: 30
            },
            {
                questionText: 'Which of the following is used to create a function in JavaScript?',
                answers: ['function = myFunc()', 'function myFunc()', 'def myFunc()', 'void myFunc()'],
                correctAnswerIndex: 1,
                selectedIndex: -1,
                seconds: 30
            },
            {
                ...
                answers: ['// this is a comment', '/* this is a comment */', '<!-- comment -->', '# comment'],
                correctAnswerIndex: 1,
                selectedIndex: -1,
                seconds: 30
            },
            {
                questionText: 'Which method is used to convert JSON to a JavaScript object?',
                answers: ['JSON.stringify()', 'JSON.parse()', 'parse.JSON()', 'JSON.toObject()'],
                correctAnswerIndex: 1,
                selectedIndex: -1,
                seconds: 30
            },
            {
                questionText: 'Which loop runs at least once even if the condition is false?',
            ...
            },
            {
                ...
                answers: ['Math.floor(4.7)', 'Math.round(4.7)', 'Math.ceil(4.7)', 'Math.trunc(4.7)'],
                correctAnswerIndex: 0,
                selectedIndex: -1,
                seconds: 30
            },
            ...
            {
                questionText: 'What is the default value of a declared but uninitialized variable?',
            ...
            {
                questionText: 'How can you convert a string to an integer in JavaScript?',
            ...
            {
                questionText: 'Which object allows you to interact with the webpage DOM?',
...
let quizIndex = 0
let lastTimeQuestionIndex = quizzes[quizIndex].quizQuestions.findIndex(question => question.selectedIndex < 0)
...
let highestPercentage = localStorage.getItem(`highestPercentageInQuiz${quizIndex}`) || 0
let highestMarks = localStorage.getItem(`highestMarksInQuiz${quizIndex}`) || '0/0'

let correctPointerPlacementAdjustment = -42
let wrongPointerPlacementAdjustment = -42

let userWasHalfWay = quizzes[quizIndex].quizQuestions.some((question) => { return [-1].includes(question.selectedIndex) }) && quizzes[quizIndex].quizQuestions.some((question) => { return [0,1,2,3].includes(question.selectedIndex) })
if (userWasHalfWay){buttonsContainer.classList.add('user-was-half-way')} else {buttonsContainer.classList.remove('user-was-half-way') }
...
function setUpQuestionOnPage(questionIndex) {
    if(questionIndex < quizzes[quizIndex].quizQuestions.length){
        ...
        currentQuestionNumberSpan.innerText = `${questionIndex + 1}/${totalQuestions}`
...
function markSelectionByUser(selectedAnswerIndex) {
    let currentCorrectIndex = quizzes[quizIndex].quizQuestions[currentQuestionIndex].correctAnswerIndex
    if (selectedAnswerIndex >= 0) {
        ...
        quizzes[quizIndex].quizQuestions[currentQuestionIndex].selectedIndex = selectedAnswerIndex
        if ([0,1,2,3].includes(selectedAnswerIndex)) {
            answers[selectedAnswerIndex].classList.add('selected')
            answers[quizzes[quizIndex].quizQuestions[currentQuestionIndex].correctAnswerIndex].classList.add('correct')
        } else {
            answers[quizzes[quizIndex].quizQuestions[currentQuestionIndex].correctAnswerIndex].classList.add('correct-unsolved')
        }
...
function nextButtonHandler (event) {
    if (currentQuestionIndex < quizzes[quizIndex].quizQuestions.length) {
        ...
        isSelected = false
        answers.forEach( (answer) => {answer.classList.remove('selected'); answer.classList.remove('correct-unsolved'); answer.classList.remove('correct');})
        console.log(`Page for question number ${currentQuestionIndex + 1} being set`);
        setUpQuestionOnPage(currentQuestionIndex)
    } else {
...
function displayResult() {
    ...
    if (correctPercentage > highestPercentage) {
        ...
        localStorage.setItem(`highestPercentageInQuiz${quizIndex}`, highestPercentage)
    ...
    correctPointerPlacementAdjustment = correctPercentage <= 10 ? -correctPercentage/3-8 : -42
    wrongPointerPlacementAdjustment = wrongPercentage <= 10 ? -wrongPercentage/3-8 : -42
    ...
    if (correctPercentage >= 90) {
        resultMotivationalQuote.innerHTML = "You didn’t just pass, you mastered it! Keep soaring!"
    } else if (correctPercentage >= 75) {
        resultMotivationalQuote.innerHTML = "You’re on the right path—just a few steps from perfect!"
    } else if (correctPercentage >= 60) {
        resultMotivationalQuote.innerHTML = "A solid attempt! Review a bit more, and you’ll level up fast."
    } else if (correctPercentage >= 50) {
        resultMotivationalQuote.innerHTML = "You’re getting there! Don’t stop now—keep practicing."
    } else if (correctPercentage >= 40) {
        resultMotivationalQuote.innerHTML = "Mistakes are proof you're trying. Reflect, revise, rise!"
    } else {
        resultMotivationalQuote.innerHTML = "Every expert was once a beginner—keep going!"
    }
